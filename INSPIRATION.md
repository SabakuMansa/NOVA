...
vérifiées en session précédente (blogs d'analyse Awwwards). Aucune référence
inventée.

---

## Les 8 références retenues

### 1. Longbow Motors — db-longbow.webflow.io (SOTD + Developer Award)
- **Sujet** : voitures électriques anglaises de luxe, philosophie « Celeritas
...
- **Sujet** : self-care apparel. **LA trouvaille structurelle** : le site est
...
- **Adaptable — idée forte pour NOVA** : raconter **« la journée d'un
  ...
  avis répondus, menu à jour). L'ancrage restaurateur devient la STRUCTURE
  du site, pas juste un argument.

### 4. Lula Oil — lulaoil.com (SOTD)
...
### 5. Neue Montréal — vu sur la grille SOTD (Demande Spéciale, SOTD + DEV)
- **Sujet** : guide de voyage officiel de Montréal — **fierté locale**.
- **Mémorable** : typographie éditoriale rouge très assumée, esprit « guide
  papier officiel » digitalisé.
- **Adaptable** : l'ancrage géographique (Île-de-France, Saint-Maur, Suresnes)
  peut être revendiqué visuellement comme une fierté éditoriale, façon guide
  local — crédibilité immédiate pour des commerçants du coin.

### 6. Depo Luxe — depoluxe.xyz (SOTD)
- **Sujet** : production publicitaire luxe.
- **Mémorable** : catalogage en **chiffres romains**, curation « musée »,
...
### 8. Références déjà vérifiées (session précédente, mêmes standards)
- **By-Kin** (SOTD + DEV + FWA, Next.js/GSAP) : scroll « pondéré », transitions
  invisibles — « une seule surface continue ». Socle technique de la refonte.
- **Mat Voyce** (SOTD, GSAP SOTY nominee) : typo cinétique timeline-driven qui
  ne bloque jamais la lecture.
- **Iventions / Minh Pham** : WebGL atmosphérique en couche de fond, jamais en
  spectacle. Un seul moment fort.
...
1. **La clarté peut être l'esthétique** (Vectr) : pas besoin de sacrifier la
   ...
   parfaitement.
2. **Une structure narrative propriétaire** vaut mieux que des effets (LunaSol) :
   le concept « la journée d'un commerçant » serait notre différenciateur de
   soumission — personne d'autre ne peut le raconter avec cette légitimité.
3. **La typographie serif dramatique** (Longbow) est le levier visuel n°1,
   compatible avec Instrument Serif déjà en place.
...
## DIRECTION VALIDÉE (réponses du 13/07/2026)

Le client a tranché en phase de questions :
- **Direction** : l'univers **AppSignal** (appsignal.com) — « plein de couleur,
...
- **Technique** : équilibré — 1 seul moment WebGL (architecture perf déjà
  ...
  reste en Lenis + Framer/CSS. Cible Lighthouse 85+.
- **Contenu** : **carte blanche totale** — copy réécrit et « La Carte »
  repensée. Les FAITS restent (prix, offres, villes, process, légitimité
  restaurateur) ; le ton devient direct et coloré. Vouvoiement conservé,
  phrases interdites toujours bannies, jamais « Uber » visible.
- **Portée** : tout d'un coup, **mobile = desktop**.

Nouvelle signature retenue (remplace la métaphore « menu de resto ») :
**« Pendant que vous servez, votre site bosse »** — le site est mis en scène
comme un petit dashboard vivant : notifications qui tombent (réservation,
avis répondu, commande), fenêtres façon terminal, badges. Le pricing devient
des « plans » dans cette UI ludique. La légitimité restaurateur reste le cœur
du discours, l'habillage devient geek et coloré.

## Piste de concept initiale (non retenue, archivée)
...
tombée du soir. Conversion : un CTA par moment de journée, ancré dans un
bénéfice concret.
