...
## [Hero interactif] Mécanique « question fixe + réponse qui défile », inspirée d'AppSignal

**Référence** : sur appsignal.com, le hero enchaîne une question fixe, un mot
qui boucle en donnant une raison différente à chaque fois, puis une phrase de
résolution. Reproduit ici avec le contenu NOVA.

### Ce qui a changé

- **« Votre site actuel, il fait quoi, là, tout de suite ? » devient le titre
  du Hero** — la plus grande typographie du site (`text-4xl sm:text-6xl
  md:text-7xl`, dépasse tous les autres titres de la page). Remplace l'ancien
  titre Hero (« Vos futurs clients vous cherchent déjà... »).
- **Réponse qui défile en dessous**, une nouvelle toutes les 1,8 s, fade +
  léger slide vertical (`opacity`/`transform` uniquement, easing partagé
  `EASE` de `components/Reveal.tsx`, jamais de rebond) : « Rien. Absolument
  rien. », « Il dort. », « Il est invisible. », « Il fait fuir vos clients. »,
  « Il tourne en rond. », « Il ment sur vos horaires. », « Il rate des
  opportunités. » — 3 dernières ajoutées dans le même esprit (courtes, ciblent
  le site, jamais le commerçant).
- **Résolution affichée en permanence en dessous** (pas de reveal différé
  après N cycles — plus simple, plus robuste, et fidèle à la référence
  AppSignal où la ligne de résolution est déjà visible pendant que la raison
  défile) : « Vos futurs clients vous cherchent déjà. Assurez-vous qu'ils
  vous trouvent. », puis les CTA inchangés.
- **Les encadrés « Il dort » / « Il est invisible » ont disparu de la section
  Constat** : leur contenu vit désormais dans le défilement du Hero. Les 2
  cartes restantes (« Il est verrouillé », « Il coûte sans compter») gardent
  leur grille, recentrée (`max-w-2xl`) pour ne pas paraître clairsemée à 2
  cartes au lieu de 4. L'ancienne ligne de rupture `v3constat.rupture`
  (« Votre site actuel... ») est retirée de Constat car elle vit maintenant
  dans le Hero — plus de doublon.
- Nouveau modèle de données `v3hero` (`content/v3.ts`) : `question`,
  `answers[]`, `resolution` remplacent `titleA`/`titleEm`/`titleB`/`subtitle`.

### Reduced motion

`RotatingAnswer` (nouveau composant dans `components/v3/Hero.tsx`) utilise
`useReducedMotion()` : si activé, affiche `answers[0]` (« Rien. Absolument
rien. » — la réponse la plus forte) statique, sans intervalle ni animation.

### Vérifications effectuées

- `tsc --noEmit` ✅, build production 15 routes ✅.
- Grep : aucune référence orpheline à `titleA`/`titleEm`/`titleB`/
  `v3hero.subtitle`/`v3constat.rupture` dans le code v3 live.
- Preview desktop : défilement observé sur plusieurs cycles (« Rien. Absolument
  rien. » → « Il est invisible. » → « Il fait fuir vos clients. » → « Il rate
  des opportunités. »), pas de saut de layout, hauteur du conteneur fixe.
- Preview mobile (375px) : question lisible sur 3 lignes, défilement + CTA
  empilés proprement, aucun débordement.
- **Reduced motion vérifié fonctionnellement** : le comportement d'émulation
  media-query du navigateur de preview n'étant pas fiable pour déclencher le
  hook en direct, le flag a été forcé temporairement dans le code
  (`useReducedMotion() || true`) le temps d'un screenshot confirmant le rendu
  statique correct (« Rien. Absolument rien. », aucune animation), puis
  immédiatement retiré — code revenu à l'état normal, `tsc` re-vérifié après
  coup.
- Section Constat : confirmé par inspection DOM que seules 2 cartes restent
  (« Il est verrouillé », « Il coûte sans compter»), les 2 autres bien
  disparues de leur ancien emplacement.
- **Aucun push** — en attente de relecture avant mise en ligne.
